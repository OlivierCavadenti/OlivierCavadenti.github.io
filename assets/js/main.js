...
(() => {
  ...
  function om(ink, x, y, s, phase, c = RED, wave = null) {
    ...
    ink.line(x, y - 9 * s, x, y - 4 * s, { w: 1.9 * s, c });
  ...
  function giantHead(ink, gx, gy, hr, t, look, rot = -0.16) {
    const g = ink.g, P = (dx, dy) => [gx + (dx * cos(rot) - dy * sin(rot)) * hr, gy + (dx * sin(rot) + dy * cos(rot)) * hr];
    for (const sx of [-1, 1]) {
      ink.draw([P(sx * 0.66, 0.05), P(sx * 1.02, -0.14), P(sx * 0.9, 0.06), P(sx * 1.02, 0.2), P(sx * 0.86, 0.26), P(sx * 0.66, 0.34)], { fill: BLUED, closed: true, w: 2 });
      for (let k = 0; k < 3; k++) ink.line(...P(sx * 0.7, 0.1 + k * 0.08), ...P(sx * 0.92, 0.0 + k * 0.1), { w: 1, alpha: 0.7 });
    }
    ...
    g.restore();
    ink.arc(...P(0, -0.55), hr * 0.3, hr * 0.08, Math.PI + 0.3, TAU - 0.3, { w: 1.2, alpha: 0.5, rot });
    const blink = (t % 5.3) < 0.2, [lx, ly] = look;
    const e1 = P(-0.34, -0.06), e2 = P(0.34, -0.06);
    eye(ink, e1[0], e1[1], hr * 0.21, lx - e1[0], ly - e1[1], blink);
    eye(ink, e2[0], e2[1], hr * 0.21, lx - e2[0], ly - e2[1], blink);
    ink.draw([P(0.02, 0.1), P(-0.04, 0.3), P(0.04, 0.32)], { w: 1.6 });
    ink.draw([P(-0.12, 0.52), P(0, 0.55), P(0.12, 0.51)], { w: 2 });
  }

  function hand(ink, hx, hy, s, t) {
    for (let k = 0; k < 4; k++) {
      const sp = spine(hx + (30 + k * 6) * s, hy - (14 - k * 7) * s, -0.9 + k * 0.28, 34 * s, -1.4, 8, 0.05, t, k);
      ink.tube(sp, (u) => (10 - u * 3) * s, { fill: BLUE, bands: 0, stip: 0.4, seed: 20 + k, w: 2 });
    }
    ink.ell(hx, hy, 58 * s, 24 * s, { fill: BLUE, w: 2.4, rot: -0.12 });
    ink.g.save(); ink.clipEll(hx, hy, 58 * s, 24 * s, -0.12)(); ink.g.clip();
    ink.stip(hx, hy + 6 * s, 58 * s, 24 * s, { seed: 9, dens: 0.8, c: BLUED });
    ink.g.restore();
    ink.tube(spine(hx - 50 * s, hy + 4 * s, -1.9, 34 * s, 1.2, 8, 0.05, t, 9), (u) => (11 - u * 3) * s, { fill: BLUE, bands: 0, stip: 0.4, seed: 31, w: 2 });
  }
  ...
  SC.planet = (api) => (t) => {
    ...
    // the giant
    const gx = 590, gy = 520 + sin(t * 0.5) * 5, hr = 150;
    ink.draw([[400, H + 10], [430, 770], [520, 700], [690, 700], [790, 760], [820, H + 10]], { fill: BLUED, closed: true, w: 2.6 });
    ink.stip(610, 880, 220, 180, { seed: 4, dens: 0.35, c: INK });
    ink.draw([[520, 700], [600, 760], [690, 700]], { w: 2 });
    ink.draw([[545, 640], [540, 710], [655, 710], [640, 640]], { fill: BLUE, closed: true, w: 2.4 });
    const hx = 400, hy = 760 + sin(t * 0.5 + 1) * 4;
    const look = api.mouse() || [hx - 30 + sin(t * 0.3) * 30, hy - 40];
    giantHead(ink, gx, gy, hr, t, look);
    // forearm and cupped hand
    ink.tube([[700, 1000], [640, 900], [560, 820], [480, 780], [430, 772]], (u) => 40 - u * 14, { fill: BLUE, bands: 0, stip: 0.7, seed: 44, side: -1, w: 2.6 });
    hand(ink, hx, hy, 1.4, t);
    om(ink, hx - 10, hy - 26, 5.2, 0, INK, t * 5);
    om(ink, hx - 10, hy - 26, 4.6, 0, RED, t * 5);
    bubble(ink, 120, 560, ['BIENVENUE,', 'TERRIEN !'], hx - 30, hy - 70, 30);
  ...
  SC.reader = (api) => (t) => {
    ...
    pages.forEach((p) => {
      ink.draw([[p.x, p.y], [p.x + 64, p.y + 4], [p.x + 62, p.y + 50], [p.x - 2, p.y + 46]], { fill: WHITE, closed: true, w: 2 });
  ...
  SC.octo = (api) => (t) => {
    ...
    for (let j = 0; j < 8; j++) {
      ...
      ink.draw([[-9, -12], [9, -12], [9, 12], [-9, 12]].map(([u, v]) => [x + u * cos(r) - v * sin(r), y + u * sin(r) + v * cos(r)]), { fill: MANA[j % 5], closed: true, w: 1.8 });
    }
    ...
    eye(ink, cx - 22, cy + 4, 13, la[0] - cx, la[1] - cy, (t % 4.9) < 0.15);
    eye(ink, cx + 22, cy + 4, 13, la[0] - cx, la[1] - cy, (t % 4.9) < 0.15);
  };
  ...
  SC.tripod = (api) => (t) => {
    ...
    for (let i = 0; i < 8; i++) {
      const hx = 44 + i * 44;
      if (i < fallen) {
        ink.draw([[hx - 13, 262], [hx - 4, 250], [hx + 7, 255], [hx + 13, 262]], { fill: WHITE, closed: true, w: 1.6 });
        const puff = (t * 20 + i * 13) % 50;
        ink.ell(hx, 244 - puff, 7 + puff * 0.18, 6 + puff * 0.14, { fill: WHITE, w: 1.2, alpha: max(0, 1 - puff / 50) });
      } else {
        ink.draw([[hx - 11, 262], [hx - 11, 246], [hx + 11, 246], [hx + 11, 262]], { fill: WHITE, closed: true, w: 1.8 });
        ink.draw([[hx - 14, 247], [hx, 233], [hx + 14, 247]], { fill: INK, closed: true, w: 1.6 });
        ink.dot(hx, 254, 2.2, YEL);
      }
    }
  ...
  SC.quill = (api) => {
    ...
    return (t) => {
      ...
      field(api, LILAC);
      ground(ink);
      ink.draw([[228, 66], [346, 69], [343, 248], [225, 245]], { fill: WHITE, closed: true, w: 2.4 });
      ink.draw([[236, 94], [336, 96], [334, 208], [234, 206]], { w: 1.4 });
      ...
      ink.draw([[112, 262], [118, 212]], { w: 3 }); ink.draw([[140, 262], [138, 214]], { w: 3 });
  ...
  SC.giant = (api) => (t) => {
    ...
    const cx = W * 0.56, cy = H * 0.44, hr = min(W, H) * 0.3;
    ink.draw([[cx - hr * 1.3, H + 10], [cx - hr * 1.05, cy + hr * 1.25], [cx + hr * 1.1, cy + hr * 1.2], [cx + hr * 1.4, H + 10]], { fill: BLUED, closed: true, w: 2.4 });
    ink.draw([[cx - hr * 0.3, cy + hr * 0.8], [cx - hr * 0.34, cy + hr * 1.25], [cx + hr * 0.36, cy + hr * 1.2], [cx + hr * 0.3, cy + hr * 0.8]], { fill: BLUE, closed: true, w: 2 });
    const hx = W * 0.27, hy = H * 0.86 + sin(t * 0.6) * 3;
    const look = api.mouse() || [hx, hy - 30];
    giantHead(ink, cx, cy, hr, t, look, -0.2);
    ink.tube([[W * 0.85, H + 20], [W * 0.62, H * 0.95], [W * 0.42, H * 0.88], [hx + 30, hy]], (u) => 20 - u * 6, { fill: BLUE, bands: 0, stip: 0.6, seed: 45, w: 2.2 });
    hand(ink, hx, hy, 0.8, t);
    om(ink, hx - 2, hy - 18, 2, 0, RED, t * 6);
    const fx = hx + 8, fy = hy - 46 - sin(t * 6) * 3;
  ...
  SC.walkers = (api) => (t) => {
    ...
    for (let i = 0; i < 4; i++) om(ink, ((t * 18 + (i * W) / 4) % (W + 40)) - 20, gy, 1.6, t * 7 + i, WHITE);
